window.onload = function() {  
  ...
  const checkboxes = document.querySelectorAll('input[type="checkbox"]');
    ...
    if(lat != null && lng != null){
      for(var i=0;i<vin.length;i++){
        if((lat == vin[i].longitude) && (lng == vin[i].latitude)){
          markersVin[i].openPopup();
          break;
        }
        else{
          for(var j=0;j<fromage.length;j++){
            if((lat == fromage[j].longitude) && (lng == fromage[j].latitude)){
              markersFromage[j].openPopup();
              break;
            }
            else{
              for(var k=0;k<escargots.length;k++){
                if((lat == escargots[k].longitude) && (lng == escargots[k].latitude)){
                  markersEscargots[k].openPopup();
                  break;
                }
              }
            }
